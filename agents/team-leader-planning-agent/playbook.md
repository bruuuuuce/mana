...
- Collect the local estimation scale, capacity constraints, and team-specific
  point conventions when available. Collect historical examples only after the
  user explicitly approves historical calibration.
...
- Historical calibration is marked `not_requested` unless the user explicitly
  approved its read-only, team-level evidence collection.

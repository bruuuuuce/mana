...
## Workflow
1. Load release scope, stories, branch diff, test evidence, and operational context.
2. Use `release-impact-summary` to identify functional, technical, stakeholder, and support impact.
3. Use `business-continuity-check` to assess cutoffs, batches, reconciliation, reporting, SLA, and manual procedures.
4. Use `incident-risk-forecast` to list likely production symptoms, detection gaps, and mitigations.
5. Use `rollback-safety` to verify rollback constraints and data compatibility.
6. Use `known-pitfalls-extraction` to include historical or local pitfalls from the service context.
7. Use `delivery-risk-radar` to aggregate owner, dependency, evidence, and escalation risks.
8. Aggregate blockers, warnings, and AM decisions needed into the expected artifacts.

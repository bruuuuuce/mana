...
jq -e '.schemaVersion=="2" and .kind=="verification-result" and .overallResult=="passed" and .checks[0].adapter=="bash_syntax" and .checks[0].trustOrigin=="framework_declared" and (.checks[0].targetFingerprint|length>10) and (.checks[0].actionFingerprint|length>10) and (.checks[0].executionFingerprint|length>10) and (.checks[0].inputDigest|length>10) and (.checks[0].executable.digest|length>10) and .checks[0].timedOut==false and .checks[0].rerunDescriptor.kind=="repository_path" and (.checks[0].adapterImplementationDigest|startswith("sha256:")) and (.checks[0].evaluationSurface|length)>=3 and .cost.modelCalls==0 and .cost.inputTokens==0 and .cost.outputTokens==0 and .judgment==null' <<<"$pass_json" >/dev/null || fail 'pass result envelope incorrect'

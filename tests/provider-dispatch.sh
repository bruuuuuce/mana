...
printf '%s\n' "${MANA_PROVIDER_ARGS[@]}" | grep -Fxq 'agents.max_depth=0' || fail 'synthesis subagents not disabled'
mana_provider_synthesis_args codex '/mana-created/scratch' model host-disposable-non-git; [ "$(join "${MANA_PROVIDER_ARGS[@]}")" = '<--ask-for-approval><never><exec><--skip-git-repo-check><--model><model><--cd></mana-created/scratch><--sandbox><read-only><--ephemeral><--ignore-user-config><--disable><multi_agent><--disable><multi_agent_v2><-c><agents.max_threads=1><-c><agents.max_depth=0><-c><agents.interrupt_message=false>' ] || fail 'disposable_non_git_codex_exec_skips_repo_check'
mana_provider_synthesis_args codex '/mana-created/scratch' model host-disposable-non-git '/host/m3-flat.schema.json'; [ "$(join "${MANA_PROVIDER_ARGS[@]}")" = '<--ask-for-approval><never><exec><--skip-git-repo-check><--output-schema></host/m3-flat.schema.json><--model><model><--cd></mana-created/scratch><--sandbox><read-only><--ephemeral><--ignore-user-config><--disable><multi_agent><--disable><multi_agent_v2><-c><agents.max_threads=1><-c><agents.max_depth=0><-c><agents.interrupt_message=false>' ] || fail 'Codex synthesis structured-output args incorrect'
mana_provider_synthesis_args codex '/mana-created/scratch' model host-disposable-non-git '/host/m3-flat.schema.json' high; [ "$(join "${MANA_PROVIDER_ARGS[@]}")" = '<--ask-for-approval><never><exec><--skip-git-repo-check><--output-schema></host/m3-flat.schema.json><--model><model><--cd></mana-created/scratch><--sandbox><read-only><--ephemeral><--ignore-user-config><-c><model_reasoning_effort="high"><--disable><multi_agent><--disable><multi_agent_v2><-c><agents.max_threads=1><-c><agents.max_depth=0><-c><agents.interrupt_message=false>' ] || fail 'Codex synthesis reasoning effort did not follow ignore-user-config'
mana_provider_synthesis_args codex '/mana-created/scratch' model host-disposable-non-git '' extreme && fail 'unsupported Codex synthesis reasoning effort was accepted'

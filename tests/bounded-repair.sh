...
[ "$(stat -c '%a' "$(dirname "$result_file")/runner/stdout.log" 2>/dev/null || stat -f '%Lp' "$(dirname "$result_file")/runner/stdout.log")" = 600 ] || fail 'raw log permissions not private'; [ "$(wc -c < "$(dirname "$result_file")/runner/stdout.log" | tr -d ' ')" -le 65536 ] || fail 'raw log cap exceeded'; ! grep -R 'SECRET_DO_NOT_TELEMETRY' "$project/.mana/runtime/events" >/dev/null || fail 'provider output leaked to telemetry'

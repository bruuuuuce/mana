# Mana Familiar consumer contract v0

Mana is the authoritative producer of Learning Journey artifacts. Mana Familiar
is a separate desktop consumer maintained in the `mana-familiar` repository;
no Flutter application code lives here.
...
Mana Familiar may also call these producer-owned operations:
...
expansion semantics. Mana Familiar owns rendering, navigation, source display,
...
schema with any such change, and coordinate a corresponding Mana Familiar
release. Mana Familiar is launched with explicit `--project-root` and
`--mana-root` arguments, so it does not rely on Mana's internal repository
layout.

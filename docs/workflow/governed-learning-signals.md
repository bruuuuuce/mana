...
`runtime observation → candidate → evidence/recurrence assessment → learning-agent review artifact → human approval → separately executed promotion`.
...
include an ID, profile/service scope, observation and category, event
references, executions, recurrence, deterministic confidence (`low`, `medium`,

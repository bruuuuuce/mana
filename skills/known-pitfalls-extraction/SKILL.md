---
name: known-pitfalls-extraction
version: 1.0.0
description: Extracts repeated mistakes from bugs, incidents, reviews, and post-merge issues.
compatibility:
  - codex
  - junie
  - claude
...
## Purpose
Turn recurring problems into explicit knowledge that future planning, review, and testing can reuse.
...
## Execution Logic
1. Cluster incidents by root cause.
2. Extract repeatable warning signs.
3. Map pitfalls to skills and rules.
4. Avoid blaming individuals; focus on process signals.
...
Use `.mana/global/known-pitfalls/` and `engineering-guards.md` as the durable learning target.
...
```mermaid
flowchart TD
    Inputs[Required inputs] --> Skill[known-pitfalls-extraction]
...
## Example Output
```yaml
skill: known-pitfalls-extraction
status: warning

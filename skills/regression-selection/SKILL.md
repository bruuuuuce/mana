...
version: 1.0.0
description: Selects targeted regression tests based on diff and risk.
compatibility:
  - codex
  - junie
  - claude
...
## Purpose
Reduce slow feedback loops by selecting a defensible subset of existing tests related to changed files and risk areas.
...
Read `testing-policy.md` and `architecture.md` to select targeted regression tests.
...
```mermaid
flowchart TD
    Inputs[Required inputs] --> Skill[regression-selection]

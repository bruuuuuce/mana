...
version: 1.0.0
description: Detects missing integration tests for DB, service, messaging, and external client interactions.
compatibility:
  - codex
  - junie
...
## Purpose
Ensure persistence, transaction boundaries, messaging, HTTP clients, and external failures are validated beyond unit tests.
...
## Execution Logic
1. Find changed integration seams.
2. Check matching IT, contract, or component tests.
3. Flag missing DB rollback or messaging scenarios.
4. Suggest fixtures or stubs.
...
Read `testing-policy.md` and `integration-map.md` to identify missing integration coverage.
...
```mermaid
flowchart TD
    Inputs[Required inputs] --> Skill[integration-test-gap]

...
name: story-depth
version: 1.0.0
description: Evaluates whether an epic or user story has sufficient and balanced detail before development starts.
compatibility:
  - codex
  - junie
  - claude
...
## Purpose
Detect incomplete or uneven analysis depth across functional goal, data requirements, validations, external calls, timeout, retry, error behavior, acceptance criteria, and open questions.
...
## Execution Logic
1. Parse the story into requirement domains.
2. Rate each domain as high, medium, low, or missing.
3. Flag asymmetry where one area is detailed but another critical area is vague.
4. Generate clarification questions before development starts.
...
Read `service-mission.md` and `domain-glossary.md` to evaluate whether the story matches service purpose and uses domain terms correctly.
...
```mermaid
flowchart TD
    Inputs[Required inputs] --> Skill[story-depth]

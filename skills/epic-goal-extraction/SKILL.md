---
name: epic-goal-extraction
version: 1.0.0
description: Extracts an epic goal contract used by planning, testing, and review.
compatibility:
  - codex
  - junie
  - claude
...
## Purpose
Convert vague epic language into a structured contract describing business goal, expected outcome, in-scope items, out-of-scope items, architectural constraints, and quality constraints.
...
## Execution Logic
1. Extract intent and expected business outcome.
2. Identify scope boundaries and exclusions.
3. Infer explicit constraints only when supported by evidence.
4. Produce YAML that downstream agents can reference.
...
The owner role `BA / PO` reviews blocker and warning findings. High-risk security, database, concurrency, or architecture findings require explicit approval before implementation or merge.

## Service Context Layer
Read `service-mission.md` and `architecture.md` to align the epic goal contract with the service purpose and architecture position.
...
```mermaid
flowchart TD
    Inputs[Required inputs] --> Skill[epic-goal-extraction]

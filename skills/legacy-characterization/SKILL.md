...
version: 1.0.0
description: Plans characterization tests for legacy flows before modifying them.
compatibility:
  - codex
  - junie
  - claude
...
## Purpose
Capture current behavior before refactoring or modifying legacy code so regressions are visible.
...
## Execution Logic
1. Identify legacy seams touched by the task.
2. Document observable behavior.
3. Propose characterization tests around current outputs.
4. Mark ambiguous behavior for team review.
...
```mermaid
flowchart TD
    Inputs[Required inputs] --> Skill[legacy-characterization]

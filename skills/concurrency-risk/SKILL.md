---
name: concurrency-risk
version: 1.0.0
description: Detects concurrency and idempotency risks.
compatibility:
  - codex
  - junie
  - claude
...
## Purpose
Find race conditions, lost updates, non-idempotent retry behavior, duplicate event processing, and unsafe shared state.
...
## Execution Logic
1. Identify shared resources and concurrent writers.
2. Review retry and duplicate delivery semantics.
3. Check locking or versioning strategy.
4. Propose tests for double submit and out-of-order events.
...
Read `architecture.md`, `integration-map.md`, and `engineering-guards.md` for shared state, event, and idempotency expectations.
...
```mermaid
flowchart TD
    Inputs[Required inputs] --> Skill[concurrency-risk]

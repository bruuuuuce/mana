---
name: liquibase-production-risk
version: 1.0.0
description: Performs production risk analysis for database schema changes.
compatibility:
  - codex
  - junie
  - claude
...
## Purpose
Detect lock risks, missing rollback, unsafe index operations, large table updates, destructive DDL, drift concerns, and traffic-aware ordering issues.
...
## Execution Logic
1. Classify each changeset by operation type.
2. Estimate operational risk using table size and traffic context when available.
3. Flag destructive or non-rollbackable operations.
4. Suggest safer sequencing and DBA approval gates.
...
The owner role `DBA / Team Leader` reviews blocker and warning findings. High-risk security, database, concurrency, or architecture findings require explicit approval before implementation or merge.

## Service Context Layer
Read `database-policy.md` and `engineering-guards.md` before assessing database risk.
...
```mermaid
flowchart TD
    Inputs[Required inputs] --> Skill[liquibase-production-risk]
...
## Example Output
```yaml
skill: liquibase-production-risk
status: warning

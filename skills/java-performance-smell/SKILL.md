---
name: java-performance-smell
version: 1.0.0
description: Detects Java performance smells in changed code.
compatibility:
  - codex
  - junie
...
## Execution Logic
1. Inspect changed loops, queries, repositories, and clients.
2. Classify smells by runtime impact.
3. Require measurement for high-risk optimization.
4. Suggest minimal safe alternatives.
...
Read `architecture.md` and `engineering-guards.md` for runtime constraints and forbidden performance patterns.
...
```mermaid
flowchart TD
    Inputs[Required inputs] --> Skill[java-performance-smell]

...
version: 1.0.0
description: Detects contradictions among stories belonging to the same epic.
compatibility:
  - codex
  - junie
  - claude
...
## Purpose
Find conflicting rules, duplicated scope, incompatible acceptance criteria, inconsistent terminology, and mismatched assumptions across related stories.
...
Read `domain-glossary.md` and `service-mission.md` to detect terminology drift and contradictory business assumptions.
...
```mermaid
flowchart TD
    Inputs[Required inputs] --> Skill[story-consistency]

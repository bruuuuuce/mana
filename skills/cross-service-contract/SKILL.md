...
version: 1.0.0
description: Reviews completeness of service, API, event, and messaging contracts.
compatibility:
  - codex
  - junie
  - claude
...
## Purpose
Check payloads, schemas, Kafka topics, error mapping, retry policy, timeout, idempotency, versioning, and ownership.
...
## Execution Logic
1. List every external interaction.
2. Verify request, response, events, errors, and retry semantics.
3. Map gaps to contract tests.
4. Escalate missing ownership or incompatible changes.
...
Read `integration-map.md` and `engineering-guards.md` before validating API, event, timeout, retry, and idempotency completeness.
...
```mermaid
flowchart TD
    Inputs[Required inputs] --> Skill[cross-service-contract]

---
name: post-merge-incident-learning
version: 1.0.0
description: Converts post-merge incidents into reusable knowledge, rules, tests, and skill updates.
compatibility:
  - codex
  - junie
  - claude
...
## Purpose
Close the loop after incidents by identifying missed signals and updating future guardrails.
...
## Execution Logic
1. Reconstruct what changed and what failed.
2. Identify why existing skills or tests missed it.
3. Propose new tests, rules, and skill improvements.
4. Route recommendations to human owners.
...
The owner role `Team Leader / Architect` reviews blocker and warning findings. High-risk security, database, concurrency, or architecture findings require explicit approval before implementation or merge.

## Service Context Layer
Use Service Context Layer files to identify which guard, rule, or policy should change after an incident.
...
```mermaid
flowchart TD
    Inputs[Required inputs] --> Skill[post-merge-incident-learning]
...
## Example Output
```yaml
skill: post-merge-incident-learning
status: warning

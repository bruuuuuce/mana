...
version: 1.0.0
description: Detects architectural risk and violations of project rules.
compatibility:
  - codex
  - junie
...
## Purpose
Review transaction boundaries, sync/async flows, idempotency, retries, feature flags, bounded contexts, and forbidden zones.
...
## Execution Logic
1. Compare proposed design or diff with architecture rules.
2. Flag synchronous cross-service calls where policies prefer async.
3. Review transaction and event-publishing boundaries.
4. Suggest safer patterns without auto-changing code.
...
Read `architecture.md`, `engineering-guards.md`, and `integration-map.md` as the local architecture rule baseline.
...
```mermaid
flowchart TD
    Inputs[Required inputs] --> Skill[architecture-risk]

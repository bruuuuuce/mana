...
for f in scripts/lib/story-start-scope-v2-govern.py scripts/lib/story-start-scope-v2-render.py tests/story-start-scope-v2-governor.sh tests/story-start-scope-v2-integration.sh; do
  if [ ! -f "$root/$f" ]; then echo "ERROR: missing $f" >&2; status=1; fi
done

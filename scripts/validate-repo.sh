...
for f in scripts/mana-compile-profile.sh tests/context-runtime-profile-compiler.sh tests/fixtures/context-runtime/codex-prompt-stub.sh contracts/context-runtime/context-manifest-v1.schema.json docs/workflow/context-profile-compilation.md; do
  if [ ! -f "$root/$f" ]; then echo "ERROR: missing $f" >&2; status=1; fi
done

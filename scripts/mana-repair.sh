...
model=""; case "$provider" in codex) model="${MANA_CODEX_MODEL:-gpt-6-luna}";; claude) model="${MANA_CLAUDE_MODEL:-haiku}";; opencode) model="${MANA_OPENCODE_MODEL:-opencode/gpt-6-luna}";; stub) model=deterministic-stub;; *) eligibility=blocked; eligibility_reason="unsupported repair runner: $provider";; esac

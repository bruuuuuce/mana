...
contains() { printf '%s\n' "$1" | grep -Fqx "$2"; }
run_scenario() {
  local dir="$1" id version profile assertions skills tools artifacts result_lines='' assertion_count=0 failed=0 line type value pass reason
  ...
  skills="$(profile_skills "$profile")"; tools="$(profile_tools "$profile" | LC_ALL=C sort -u)"; artifacts="$(profile_artifacts "$profile" | LC_ALL=C sort -u)"
  ...
  while IFS='|' read -r type value; do
    [ -n "$type" ] || continue; assertion_count=$((assertion_count + 1)); pass=true; reason=""
    case "$type" in
      must_use_skill) contains "$skills" "$value" || { pass=false; reason="skill is not selected by profile"; };;
      must_not_use_tool) contains "$tools" "$value" && { pass=false; reason="tool is allowed by profile agents"; } || reason="tool is not allowed";;
      must_require_gate) grep -Fqi "$value" "$root/profiles/$profile.yaml" || { pass=false; reason="required gate or blocker is absent"; }; [ -n "$reason" ] || reason="gate or blocker is declared";;
      must_produce_artifact) contains "$artifacts" "$value" || { pass=false; reason="artifact is not declared by profile agents"; };;
      must_stop_with) grep -Fqi "$value" "$root/profiles/$profile.yaml" || { pass=false; reason="stop condition is absent"; };;
      must_not_modify) [ "$value" = true ] || { pass=false; reason="must_not_modify only accepts true"; }; [ -n "$reason" ] || reason="evaluation is plan-only and read-only";;
      ...
      must_flag) grep -Fqx "$value" "$dir/fixture-signals.txt" 2>/dev/null || { pass=false; reason="fixture does not provide required risk signal"; };;
      must_not_flag) ! grep -Fqx "$value" "$dir/fixture-signals.txt" 2>/dev/null || { pass=false; reason="fixture provides forbidden risk signal"; };;
      *) pass=false; reason="unsupported assertion type";;
    esac
    [ "$pass" = true ] || failed=$((failed + 1))
    result_lines="${result_lines}${result_lines:+$'\n'}$type|$value|$pass|$reason"
  done <<EOF
$assertions
EOF
...
write_result() {
  local id="$1" version="$2" profile="$3" status="$4" count="$5" failed="$6" lines="$7" out dir rev first=true type value pass reason selected gates artifacts
  rev="$(revision)"; dir="$project_root/.mana/evaluations/results"; mkdir -p "$dir"; out="$dir/$id-v$version-$rev.json"
  ...
  { printf '{"schemaVersion":"1","scenarioId":"%s","scenarioVersion":"%s","manaRevision":"%s","profile":"%s","status":"%s","pass":%s,"durationMs":0,"modelRouting":[],"runtimeWarnings":[],"evidenceReferences":' "$(escape "$id")" "$(escape "$version")" "$(escape "$rev")" "$(escape "$profile")" "$status" "$([ "$status" = passed ] && printf true || printf false)"
    ...
    while IFS='|' read -r type value pass reason; do [ -n "$type" ] || continue; "$first" || printf ','; first=false; printf '{"type":"%s","value":"%s","pass":%s,"reason":"%s"}' "$(escape "$type")" "$(escape "$value")" "$pass" "$(escape "$reason")"; done <<EOF
$lines
EOF
    printf ']}'$'\n'; } > "$out"
  printf '%s\n' "$out"
...
EOF
  ...
  lines="$(printf '%s\n' "$record" | sed '1d')"; out="$(write_result "$id" "$version" "$profile" "$status" "$count" "$failed" "$lines")"; results="${results}${results:+$'\n'}$out"; [ "$status" = passed ] || overall=false
done <<EOF
$dirs
EOF
if [ "$json" = true ]; then printf '{"status":"%s","results":' "$([ "$overall" = true ] && printf passed || printf failed)"; jlist <<<"$results"; printf ',"readOnly":true}\n'; else echo 'MANA EVAL'; printf '%s\n' "$results"; [ "$overall" = true ] && echo 'All deterministic assertions passed.' || echo 'One or more deterministic assertions failed.'; fi
[ "$overall" = true ]

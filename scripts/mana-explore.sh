#!/usr/bin/env bash
set -u
root="$(cd "$(dirname "$0")/.." && pwd)"; project_root="$(pwd)"; question=""; json=false; scope=all
usage() { echo 'Usage: mana explore "<question>" [--scope all|repository|service-context|user-context] [--json]' >&2; }
while [ "$#" -gt 0 ]; do case "$1" in --project-root) project_root="${2:-}"; shift 2;; --scope) scope="${2:-}"; shift 2;; --json) json=true; shift;; --help|-h) usage; exit 0;; --*) echo "ERROR: unknown option: $1" >&2; exit 2;; *) [ -z "$question" ] || { echo 'ERROR: only one question is accepted' >&2; exit 2; }; question="$1"; shift;; esac; done
[ -n "$question" ] || { usage; exit 2; }
. "$root/scripts/lib/user-context.sh"
. "$root/scripts/lib/explorer-retrieval.sh"
explorer_retrieve "$project_root" "$question" 3 "$scope"
